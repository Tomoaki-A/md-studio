...
const getPlanContent = ({
  ...
  const planPathList = getPlanList({
    rootPath,
  });
  const normalizedTarget =
    ...
    isPathAllowed({
      ...
      : undefined;
  const selectedPath =
    normalizedTarget && planPathList.includes(normalizedTarget)
      ? normalizedTarget
      : planPathList[0];
